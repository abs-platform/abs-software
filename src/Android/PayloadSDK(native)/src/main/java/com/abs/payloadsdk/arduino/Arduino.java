...
public class Arduino {
    ...
    public EventHandler createEvent(int interval, Event.ACTIONS action , byte[] args)
    {
        byte[] cmd_args = {(byte)interval};

        Event event = new Event(action, args, null);

        Log.d("[ABS]","Event here ok1");

        SDBPacket response = sdb.sendSync(new SDBPacket(
                SDBPacket.CMD.CREATE_EVENT, cmd_args, event.toRaw()));

        Log.d("[ABS]","Event here ok2");
